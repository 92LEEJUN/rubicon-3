...
function mockFetch() {
  (global as any).fetch = jest.fn(() =>
    Promise.resolve({
      ok: true,
      status: 200,
      json: async () => ({ primary_label: '부품 입고', primary_ref: 'r1' }),
    }),
  );
}
afterEach(() => {
...
test('동의 철회 시 노출 중 배너 즉시 제거(요구 6.3)', async () => {
  ...
  const { result } = renderHook(() => useReEngagement({ base: 'https://bff.test' }, true), {
    wrapper: Wrapper,
  });
...
test('useChat.resumeFromRef — 임의의 ChatTransport(stub)에만 의존, ref를 screen_context로 주입(요구 2.2·5.1)', () => {
  ...
  act(() => {
    result.current.resumeFromRef('loop-42', { screen: 'home' });
  });
  ...
  const script = (_m: ClientMessage): Chunk[] => [
    { type: 'delta', text: '안녕' },
    {
      ...
      section: {
        ...
        handled: true,
        ctas: [],
        template: { kind: 'text', data: { message: 'x' } },
      },
    },
