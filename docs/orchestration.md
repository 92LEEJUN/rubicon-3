...
## 11. 슈퍼바이저 양끝(plan + compose) + 병렬 가드레일 (ADR-0053·0054)

capability 오케스트레이터(②)는 기본적으로 **LLM 플래너가 라우팅만** 하고 각 핸들러가 결정론으로
섹션을 뱉는다("데이터는 결정론, 말은 LLM"). 복합 턴에서 응답이 카드 나열처럼 보이는 한계를 보완하려고,
**LLM 플래너를 슈퍼바이저로 승격**해 턴의 양끝을 잡는다(`astream` 경로, 토글 뒤).

- **plan(앞)** — 의도 추출/라우팅. 기존과 동일(ADR-0048).
- **compose(뒤)** — 핸들러 섹션의 **facts**를 받아 **자연어 내러티브만** 종합한다. **카드·CTA·게이팅·
  data는 재생성하지 않고** 그대로 통과(환각·계약 훼손 방지). 처리 섹션이 2개 미만이면 스킵, 실패하면
  내러티브를 생략(카드로 완결). `COMPOSE` off면 오늘과 동일(회귀 불변).
- **2-track 스트리밍(ADR-0055)** — 결정적 **카드 섹션을 compose 완료 전에 먼저** 방출(`section*`)하고,
  내러티브는 뒤따라 **`delta`로 스트리밍**한다. FE가 delta를 별도 슬롯(상단)에 누적하므로 방출
  순서(카드 먼저)와 표시 순서(내러티브 위)가 분리 → **first-token = 라우팅 홉**으로 회복. `GUARDRAIL`
  on이면 내러티브를 완성 후 마스킹(버퍼, 안전 우선), off면 토큰 스트리밍.
- **가드레일(병렬·취소)** — 입력 검사(pre)를 **라우팅과 병렬 task**로(직렬 지연 0). 차단 시 라우팅
  task를 **취소**하고 안전 거부 1섹션(**fail-closed**, 예외도 차단). 방출 직전 출력 검사(post)로 텍스트
  PII 마스킹(구조 필드 불변). `GUARDRAIL` off면 미발동. 결정적 규칙(LLM 무관).

> **레이턴시.** 2-track 적용으로 compose 턴 first-token이 ~라우팅 홉으로, 차단 턴은 ~screen으로 내려간다.
> 총 E2E는 내러티브 완료까지라 compose 턴이 길지만 delta 점진 방출로 체감 대기가 짧다. 멀티턴 실측은
> `backend/verify_multiturn_timing.py`(단발은 `verify_compose_timing.py`). 근거: ADR-0053·0054·0055.
