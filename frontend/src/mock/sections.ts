...
const ctaVisit: Cta = {
  ...
  action: 'chat',
  kind: 'booking',
  payload: { visit_type: 'REPAIR' },
};
const orderCta = (ids: string[]): Cta => ({
  label: '주문하기',
  action: 'commit',
  kind: 'order',
  payload: { part_ids: ids },
});
...
export function buildDiagnose(text: string): MessageSection[] {
    ...
    return [
      {
        ...
        template: {
          ...
          data: {
            cta_notice:
              '말씀하신 증상은 안전 위험이 있을 수 있어요. 사용을 멈추고 전원(또는 가스)을 차단한 뒤, 직접 손대기보다 상담원·수리기사 방문으로 점검받으시길 권해요.',
  ...
  if (inWarranty)
    data.cta_notice =
      '보증 기간 내 무상 수리 대상이에요. 부품을 직접 구매하기보다 보증 수리(상담원·방문)를 이용하시는 걸 권해요.';
  return [
    {
      label: '해결 가이드',
      intent: 'troubleshoot',
  ...
  return resolvePartIds(text).map((pid) => {
    ...
    return {
      ...
      template: {
        kind: 'text',
        data: {
          message: `'${part?.name ?? pid}'은(는) 현재 품절입니다. 입고 알림을 신청하거나 대체 제품을 안내해 드릴게요.`,
          part_id: pid,
        },
      },
      ctas: [
        { label: '입고 알림', action: 'chat', kind: 'restock_alert', payload: { part_id: pid } },
  ...
  return [
    {
      ...
      ctas: [
        { label: '자세히', action: 'chat', kind: 'explain' },
        { label: '비교', action: 'chat', kind: 'compare' },
      ],
    },
  ];
}
  ...
  return [
    {
      ...
      intent: 'warranty',
      handled: true,
      template: { kind: 'text', data: { message, coverage: free ? 'free' : 'unknown' } },
      ctas,
    },
  ];
}
  ...
  return [
    {
      ...
      ctas: SLOTS.slice(0, 3).map((s) => ({
        label: `${s.start} 예약`,
        action: 'commit',
        kind: 'booking',
        payload: { slot_id: s.id },
      })),
    },
  ];
}
  ...
  return [
    {
      ...
      template: {
        ...
        data: { products: chosen, detail: true, personalized: true },
      },
      ctas: [
        {
          label: '장바구니',
          action: 'commit',
          kind: 'order',
          payload: { product_ids: chosen.map((p) => p.id) },
        },
      ],
    },
  ];
  ...
  return [
    {
      ...
      template: {
        kind: 'text',
        data: { message: '어떤 기기의 어떤 점이 궁금하신지 알려주시면 정확히 도와드릴게요.' },
      },
      ctas: DEVICES.slice(0, 3).map((d) => ({
        label: d.type,
        action: 'chat',
        kind: 'select_device',
