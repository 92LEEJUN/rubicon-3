...
**추천(잠정): C** — MVP는 B로 시작하되, 핵심 의도(주문·핸드오프)에 규칙 가드레일을 덧댄다.
...
**추천(잠정): B** — MVP의 `TrustPort`는 Mock이나 인터페이스는 RAG 전제로 둔다. 근거 없으면 핸드오프(R18).
...
**추천(잠정): B** — 요구사항 R4-3(임의 선택 금지)과 정합.
...
**추천(잠정): C** — 미처리 의도는 `IntentResult.unhandled`로 구분 응답.
...
**추천(잠정): C** — `docs/data-model.md`의 `FlowState`와 정합.

> 위 추천은 **잠정**이며, 6.1·6.2·6.5·6.7 등 영향이 큰 항목은 사용자 확정 후 본문에 반영한다.
