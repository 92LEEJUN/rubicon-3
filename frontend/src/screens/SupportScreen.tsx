    ...
    <View style={styles.root} testID="screen-support">
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Card>
          {devices.map((d: any, i: number) => (
            <View key={i} style={[styles.deviceRow, i < devices.length - 1 && styles.rowDivider]}>
              ...
              />
              <Pressable
                testID={`cs-fix-${d.type}`}
                onPress={() => onAsk?.(`${DEVICE_KO[d.type] ?? d.type} 문제 해결`)}
                style={styles.fixBtn}
              >
                <Caption>해결</Caption>
              </Pressable>
        ...
        <Pressable testID="cs-activity" onPress={() => onAsk?.('내 주문 진행 상태 알려줘')}>
          <Card>
            <TemplateView template={statusTracker} />
          </Card>
        </Pressable>
        ...
        {recs.length ? (
          <>
            ...
            <Pressable testID="cs-recommend" onPress={() => onAsk?.('추천 제품 자세히 알려줘')}>
              <Card>
                <TemplateView
                  template={{ kind: 'recommendation_list', data: { products: recs } }}
                />
              </Card>
            </Pressable>
          </>
