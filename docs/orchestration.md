...
- LLM은 **Claude** (`architecture.md` §2). 기본 모델 `claude-opus-4-8`, 단순·대량 처리는
  `claude-sonnet-4-6`/`claude-haiku-4-5`로 라우팅(비용/지연 균형).
- 호출은 단일 **Messages API**(`/v1/messages`) 기반. **tool use·구조화 출력·스트리밍**은
  별도 API가 아니라 이 호출의 기능이다.
...
수신(user_message)
  ...
  → 3) tool 선택·실행   (Claude tool use; 구현은 함수/DB/Port — §3)
...
- Claude tool use 메커니즘: 응답 `stop_reason: "tool_use"` → 우리가 실행 → `tool_result` 회신 → 반복.
...
- **구조화 출력**으로 분류+분해를 한 번에 받는다(Claude `output_config.format` = json_schema 또는 strict tool).
  ```python
  # 출력 스키마(의사)
...
generate  → Claude로 답변 + 출처 표기(TrustP)
```
...
- **adaptive thinking + effort** — 복잡 추론은 `thinking: adaptive`, `effort`로 깊이 조절.
  단순 분류/조회는 effort 낮춰 비용 절감.

## 7. 스트리밍 / 세션

- 응답은 **스트리밍 우선**(R14). Claude 스트림(SSE) → 오케스트레이터가 청크를 클라이언트 계약
...
- 서버사이드 tool 루프 한계 시 `pause_turn`은 재요청으로 이어간다.
