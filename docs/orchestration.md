...
- **compose(뒤)** — 핸들러 섹션의 **facts**를 받아 **자연어 내러티브만** 종합해 선두에 둔다.
  **카드·CTA·게이팅·data는 재생성하지 않고** 그대로 통과(환각·계약 훼손 방지). 처리 섹션이 2개
  미만이면 스킵, 실패하면 원본 섹션으로 폴백. `COMPOSE` off면 오늘과 동일(회귀 불변).
- **가드레일(병렬)** — 입력 검사(pre)를 **라우팅과 `asyncio.gather`로 병렬**(직렬 지연 0). 차단 시
  capability를 스킵하고 안전 거부 1섹션(**fail-closed**, 예외도 차단). 방출 직전 출력 검사(post)로
  텍스트 PII를 마스킹(구조 필드 불변). `GUARDRAIL` off면 미발동. 결정적 규칙(LLM 무관).

> **레이턴시 트레이드오프.** compose는 **전 섹션 수집(배리어) + LLM 1콜**이라 **first-token·총 E2E가
> 증가**한다(의도된 비용 — 품질 우선). 2-track(카드 선-전송)·텍스트 섹션 접기는 후속 최적화. 실측은
> `backend/verify_compose_timing.py`. 결정 근거는 ADR-0053·0054.
