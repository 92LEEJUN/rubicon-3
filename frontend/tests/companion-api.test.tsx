...
test('getResume — has_context payload를 그대로 돌려준다(요구 1.1)', async () => {
  mockFetch(() => ({
    json: () => ({ has_context: true, summary: '세탁기 5C', elapsed_label: '어제' }),
  }));
...
test('getReEngagement — deliver=true면 POST /reengagement/deliver(요구 3.2)', async () => {
  ...
  mockFetch((url, init) => {
    method = init?.method;
    path = url;
    return { json: () => ({ primary_label: '부품 입고' }) };
  });
...
test('postOpenLoopAction — 성공 시 ok(요구 2.4)', async () => {
  mockFetch((url) => {
    expect(url).toContain('/open-loops/ref1/resolve');
