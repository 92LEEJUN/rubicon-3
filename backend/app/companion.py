...
from .domain import ConversationMemory, ResumePayload
from .repositories import InMemoryConversationMemoryRepository, InMemoryConversationStore
...
class CompanionService:
    ...
    def resume(self, user_id: str, *, fresh: bool = False,
        ...
        has_context = bool(mem.summary or mem.facts or self.store.messages(user_id) or suspended_flow)
        return ResumePayload(has_context=has_context, summary=mem.summary, facts=mem.facts,
                             elapsed_label=elapsed, suspended_flow=suspended_flow)

    def forget(self, user_id: str) -> None:
        """삭제 요청 cascade(R19) — 메모리 + 워킹 메시지."""
