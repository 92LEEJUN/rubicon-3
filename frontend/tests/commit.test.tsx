  ...
  mockFetch((url) => {
    ...
    return {
      ...
      json: () => ({
        ...
        template: { kind: 'confirmation', data: { x: 1 } },
      }),
    };
  });
...
test('409 → 2-step 재제출은 confirmed:true 동반, 200이면 ok', async () => {
  ...
  mockFetch((_url, init) => {
    ...
    return lastBody.confirmed
      ? { status: 200, json: () => ({ order_id: 'o1' }) }
...
test('401 LoginRequired → login 상태 + login CTA', async () => {
  mockFetch(() => ({
    ...
    json: () => ({
      code: 'LoginRequired',
      cta: { kind: 'login', action: 'navigate', label: '로그인' },
    }),
  }));
...
test('booking commit은 /bookings로 POST', async () => {
  ...
  mockFetch((url) => {
    path = url;
    return { status: 200, json: () => ({ booking_id: 'b1' }) };
  });
