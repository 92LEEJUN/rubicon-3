...
> **종합 내러티브(compose, ADR-0053·0055).** 슈퍼바이저가 복합 턴의 섹션들을 자연어로 정리한 내러티브는
> **신규 kind/섹션 없이 기존 `delta` 청크**로 방출한다(2-track: 카드 `section*`를 먼저, 내러티브 `delta`를
> 뒤에). FE는 delta 누적 텍스트를 섹션 스택과 **별도 슬롯(상단)** 에 이미 렌더 → **계약 추가 없음**.
> 내러티브는 뒤따르는 카드/CTA를 "참조"만 하고 데이터를 재기술하지 않는다(카드 섹션이 권위 있는 출처).
