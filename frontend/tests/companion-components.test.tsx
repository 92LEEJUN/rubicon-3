...
test('ResumeCard — 요약·상대시간·open-loop·이어가기/새로시작 렌더(요구 1.1~1.4)', () => {
  ...
  render(
    <ResumeCard
      resume={resume}
      loops={sortByPriority(loops)}
      onContinue={onContinue}
      onStartFresh={onFresh}
    />,
  );
...
test('OpenLoopItem — kind 라벨·탭 재진입·resolve/dismiss(요구 2.1~2.3)', () => {
  const onOpen = jest.fn(),
    ...
    onDismiss = jest.fn();
  render(
    <OpenLoopList loops={loops} onOpen={onOpen} onResolve={onResolve} onDismiss={onDismiss} />,
  );
  ...
  const sections: MessageSection[] = [
    {
      ...
      handled: true,
      ctas: [],
      template: { kind: 'text', data: { message: '5C' } },
    },
    {
      label: '주문',
      intent: 'order',
      handled: false,
      ctas: [],
      template: { kind: 'unknown_kind', data: {} },
    },
  ];
